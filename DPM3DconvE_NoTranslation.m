function [data] = DPM3DconvE_NoTranslation(defaultN, volFrac, varargin)
%% Input Parsing
...
J(1) = 1/5*defaultrhok*V*(b^2 + c^2); %Moment of inertia of particles along a axis
J(2) = J(1); %Moment of inertia of particles along b axis
J(3) = 1/5*defaultrhok*V*(a^2 + b^2); %Moment of inertia of particles along c axis
...
%while t<i*0.0001 && maxDeflection < 10
    ...
    % Create a set of 4 k-values (pages) for the Runge-Kutta 4 approximation of the particle direction, with a
    % vector (cols) for each of the N particles (rows)
    kD = zeros(3, N, 4);
    
    % Create a set of 4 k-values (pages) for the Runge-Kutta 4
    % approximation for the angular velocity,
    % with a vector (cols) for each of the N particles (rows)
    kW = zeros(3, N, 4);
    ...
    for j = 1:4
        ...
        % Determine a scaling factor for time step for direction Runge Kutta approximation
        if j == 1
            timeScale = 0;
        elseif j < 4
            timeScale = 0.5;
        ...
        % Create a temporary direction for use in calculation of k values for
        % Runge Kutta
        tempD = D + h*timeScale*kD(:, :, fibonacci(j + 1));
        tempD = tempD./(ones(3,1)*sum(tempD.^2).^(1/2));
    
        Dnx = ones(N,1)*tempD(1,:);
        Dny = ones(N,1)*tempD(2,:);
        Dnz = ones(N,1)*tempD(3,:);
    
        Dmx = tempD(1,:)'*ones(1,N);
        Dmy = tempD(2,:)'*ones(1,N);
        Dmz = tempD(3,:)'*ones(1,N);
        ...
        for iz = 1:N
            ...
            rzq(iz,:) = vrrotvec(zold,tempD(:,iz,1)); % find rotation axis and angle
...
%         Tmz = mu0*M*V.*(Dnx(1, :).*Hy - Dny(1, :).*Hx);
        ...
        %Transform omega from global to local
        omegalocx = dot(xnew, tempOmega', 2);
        omegalocy = dot(ynew, tempOmega', 2);
        omegalocz = dot(znew, tempOmega)';

        % Calculate the drag torque on each particle
%         Tdx = 8*pi*eta*R^3.*tempOmega(1, :);
%         Tdy = 8*pi*eta*R^3.*tempOmega(2, :);
%         Tdz = 8*pi*eta*R^3.*tempOmega(3, :);
        Tdx = 8*pi*eta*R^3.*omegalocx;
        Tdy = 8*pi*eta*R^3.*omegalocy;
        Tdz = 8*pi*eta*R^3.*omegalocz;

%         % Calculate the total local torque on each particle
%         Tx = Tmx - Tdx + Tnewx;
%         Ty = Tmy - Tdy + Tnewy;
%         Tz = Tmz - Tdz + Tnewz;
        Tlocx = Tmx - Tdx + Tex;
        ...
        alphaglobx = alphalocx.*xnew(:,1) + alphalocy.*ynew(:,1) + alphalocz.*znew(1,:)';
        alphagloby = alphalocx.*xnew(:,2) + alphalocy.*ynew(:,2) + alphalocz.*znew(2,:)';
        alphaglobz = alphalocx.*xnew(:,3) + alphalocy.*ynew(:,3) + alphalocz.*znew(3,:)';
        ...
        % Calculate the vector of k for each particle
%         kW(1, :, j) = T(1, :);
%         kW(2, :, j) = T(2, :);
%         kW(3, :, j) = T(3, :);
        kW(1, :, j) = alpha(1, :);
        ...
        kD(1, :, j) = tempOmega(2, :).*tempD(3, :) - tempOmega(3, :).*tempD(2, :);
        kD(2, :, j) = tempOmega(3, :).*tempD(1, :) - tempOmega(1, :).*tempD(3, :);
        kD(3, :, j) = tempOmega(1, :).*tempD(2, :) - tempOmega(2, :).*tempD(1, :);
    end
    
    % Calculate the new omega
    omega = omega + 1/6*h*(kW(:, :, 1) + 2*kW(:, :, 2) + 2*kW(:, :, 3) + kW(:, :, 4));
    
    %Calculate the new D
    D = D + 1/6*h*(kD(:, :, 1) + 2*kD(:, :, 2) + 2*kD(:, :, 3) + kD(:, :, 4));
...
data=struct('Parameters',[N,vol,HtoM,R,dim,M,eta],'X',Xdata,'D',Ddata,'g',g,'f',f,'t1',tdata,'E0',E0,'H0',H0,'dim',dim,'pglobx',pglobx,'pgloby',pgloby,'pglobz',pglobz);

%% Runge-Kutta Iterative Solver
